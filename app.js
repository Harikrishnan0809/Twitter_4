...
app.get("/user/tweets/feed/", authenticateToken, async (request, response) => {
  ...
  const query = `SELECT u.username, t.tweet, t.date_time FROM user u 
    JOIN follower f ON f.following_user_id = u.user_id
...
app.get("/user/followers/", authenticateToken, async (request, response) => {
  ...
  const userId = await db.get(userIdQueryy);

  const query = `SELECT DISTINCT u.name
    FROM user u JOIN follower f ON u.user_id = f.follower_user_id
    WHERE f.following_user_id = ${userId.user_id}`;
  const tweetsArray = await db.all(query);
  response.send(tweetsArray);
});
...
app.get("/tweets/:tweetId/", authenticateToken, async (request, response) => {
  ...
  const tweet = await db.get(tweetsQuery);
  let userIdQuery = `SELECT user_id FROM user WHERE username="${username}"`;
  const userId = await db.get(userIdQuery);

  let userFollowersQuery = `SELECT * FROM follower f
  ...
  } else if (userFollowers.some((obj) => obj.user_id === tweet.user_id)) {
...
app.get(
  ...
  async (request, response) => {
    ...
    if (userFollowers.some((obj) => obj.user_id === tweet.user_id)) {
      const query = `SELECT u.username
          FROM user u 
          JOIN like l ON u.user_id = l.user_id
          WHERE l.tweet_id = ${tweetId}`;
      const tweetsArray = await db.all(query);
      const userNamesList = tweetsArray.map((each) => each.username);
      response.send(sendLikes(userNamesList));
    } else {
      response.status(401);
      response.send("Invalid Request");
    }
